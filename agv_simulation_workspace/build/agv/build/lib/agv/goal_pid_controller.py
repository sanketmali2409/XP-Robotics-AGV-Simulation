...
class GoalPIDController(Node):
    ...
    def goal_callback(self, msg):
        ...
        if getattr(self, 'first_goal_received', False) == False:
            ...
            self.csv_writer.writerow(['Timestamp', 'Target_X', 'Target_Y', 'Actual_X', 'Actual_Y', 'Error', 'PWM_Command', 'Kp', 'Ki', 'Kd'])
    ...
    def control_loop(self):
        ...
        if abs(error) >= self.distance_tolerance or abs(angle_error) > 0.1:
            ...
            # Only drive forward if we are roughly facing the goal (within ~30 degrees) to prevent drifting
            ...
            if abs(angle_error) > 0.05:
                min_turn_power = 1.0  # Increased minimum rad/s required to overcome floor friction
                if angular_cmd > 0:
