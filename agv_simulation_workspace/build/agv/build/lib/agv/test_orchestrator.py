...
class TestOrchestrator(Node):
    ...
    def goal_response_callback(self, future):
        ...
        # Start a 120-second maximum travel timeout for this goal (in case sim runs slow)
        self.timeout_timer = self.create_timer(120.0, lambda: self.on_goal_timeout(goal_handle))
    ...
    def on_goal_timeout(self, goal_handle):
        ...
        self.get_logger().warn(f'TIMEOUT: Failed to reach Waypoint {self.current_phase + 1} within 120 seconds. Canceling goal...')
        # Try to cancel the goal on the action server. 
        # We do NOT call advance_phase() here. The get_result_callback will receive the canceled status and advance cleanly.
